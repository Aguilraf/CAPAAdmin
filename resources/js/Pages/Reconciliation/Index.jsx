...
import DailyIncomeDetails from './DailyIncomeDetails';
import PendingInvoicesModal from './PendingInvoicesModal';

const money = new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN' });

export default function Index({ policies, selectedPolicy, dailyIncomes, dailyIncomeSummaries, selectedDailyIncome, invoicesList, stats, filters }) {
    ...
    useEffect(() => {
        const initial = invoicesList
            .filter(inv => filters.without_income
                ...
                : inv.daily_income_id === Number(dailyIncomeId))
            .map(inv => inv.id);
    ...
    const handleDay = (e) => {
        ...
        const hasUnsavedChanges = selectedInvoices.length !== savedSelection.length
            || selectedInvoices.some(id => !savedSelection.includes(id));

        if (hasUnsavedChanges && !window.confirm('Hay cambios de facturas sin guardar. ¿Deseas cambiar de día y descartar esos cambios?')) {
            return;
        }
        ...
        <AuthenticatedLayout header={<h2 className="text-xl font-bold text-gray-800">Conciliación Facturas Vs Ingresos</h2>}>
            ...
            <div className="py-6 px-4 max-w-7xl mx-auto space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-4 shadow-sm cursor-pointer hover:border-yellow-400 transition-colors" onClick={() => setShowPendingModal(true)}>
                ...
                <DailyIncomeDetails dailyIncomes={dailyIncomeSummaries || []} policy={selectedPolicy} />
            </div>
            ...
            {showPendingModal && (
                <PendingInvoicesModal
                    ...
                    onResolved={() => router.reload({ only: ['stats', 'invoicesList'] })}
                />
            )}
        </AuthenticatedLayout>
    );
